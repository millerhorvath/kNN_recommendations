...
# k-fold cross-validation with 5 subsamples
for i in range(5):
    t_start = time.time()
    ...
    if os.path.exists(DEF_FOLDER+'similar_matrices/my_item_similar{}.pkl'.format(i)):
        # Loading similarity matrix from file
        t_start = time.time()
        item_similar = item_knn_obj.compute_similarity_matrix(1, k, True, False, True, DEF_FOLDER+'similar_matrices/my_item_similar{}.pkl'.format(i))
        t_end = time.time()
        ...
        if not os.path.exists(DEF_FOLDER+'similar_matrices'):
            os.makedirs(DEF_FOLDER+'similar_matrices')
            
        pickle.dump(item_similar, open(DEF_FOLDER+'similar_matrices/my_item_similar{}.pkl'.format(i), 'wb'))
    
    # Computing RMSE, MAE and Coverage
    t_start = time.time()
    (rmse, mae, coverage) = item_knn_obj.root_mean_squared_error(DEF_FOLDER+'test/myTest{}.csv'.format(i), True, DEF_FOLDER+'eval/myEval{}.csv'.format(i), item_similar)
    t_end = time.time()
...
f = open(DEF_FOLDER+'results.csv', "w")
